    ...
    public partial class adminForm : Form
    {
        ...
        private void adminForm_Load(object sender, EventArgs e)
        {
            ...
            using (var context = new ExSysContext())
            {
                ...
                listBoxTrackes.DataSource = tracks;
                listBoxTrackes.DisplayMember = "TrackName";
            ...
            using (var context = new ExSysContext())
            {
                ...
                List<Instructor> Supervisors = new List<Instructor>();
               
                foreach (var track in tracks)
                {
                    foreach (var instructor in instructors)
                    {
                        if (instructor.InstructorId == track.SupervisorId)
                        {
                            Supervisors.Add(instructor);
                        }
                    }
                }
                List<Instructor> NotSupervisors = instructors.Except(Supervisors).ToList();
                var DataSource = NotSupervisors.Select(i => new { i  ,i.InstructorId, fullName = i.InstructorFname + " " + i.InstructorLname }).ToList();
                listBoxInst.DataSource = NotSupervisors;
                listBoxInst.DisplayMember = "InstructorFname";
                listBoxInst.ValueMember = "InstructorId";
                listBoxInst.SelectedIndex = 0;
            }
        }
        ...
        private void buttonAddBranch_Click(object sender, EventArgs e)
        {
            try
            {
                string branchName = textBoxBranchName.Text.Trim();
                ...
                using (var context = new ExSysContext())
                {
                    ...
                    int lastBranchId = context.Branches.OrderByDescending(b => b.BranchId).Select(b => b.BranchId).FirstOrDefault();

                    // Add a new branch with ID one greater than the last ID
                    var newBranch = new Branch { BranchId = lastBranchId + 1, BranchName = branchName };
                    context.Branches.Add(newBranch);
                    context.SaveChanges();
                }

                // Refresh the list of branches after adding a new branch
                RefreshBranchList();

                MessageBox.Show($"New branch added: {branchName}");
            }
            catch (DbUpdateException ex)
            {
                MessageBox.Show("Error saving branch to database. See details: " + ex.InnerException.Message);
            }
            catch (Exception ex)
            {
        ...
        private void RefreshBranchList()
        {
            ...
            {
                var branches = context.Branches.ToList();
                listBoxBranches.DataSource = null;
                listBoxBranches.DataSource = branches;
                listBoxBranches.DisplayMember = "BranchName";
            }
        }
        ...
        private void buttonTrackes_Click(object sender, EventArgs e)
        {
            try
            {
                string trackName = textBoxTrackName.Text.Trim();
                int sup_id =((Instructor) listBoxInst.SelectedItem).InstructorId;
                ...
                using (var context = new ExSysContext())
                {
                    var newTrack = new Track { TrackName = trackName , SupervisorId = sup_id };
                    context.Tracks.Add(newTrack);
                    context.SaveChanges();
                }

                // Refresh the list of tracks after adding a new track
                RefreshTrackList();

                MessageBox.Show($"New Track added: {trackName}");
            }
            catch (DbUpdateException ex)
            {
                MessageBox.Show("Error saving Track to database. See details: " + ex.InnerException.Message);
            }
            catch (Exception ex)
            {
                MessageBox.Show("An unexpected error occurred: " + ex.Message);
        ...
        private void RefreshTrackList()
        {
            ...
            {
                var tracks = context.Tracks.ToList();
                listBoxTrackes.DataSource = null;
                listBoxTrackes.DataSource = tracks;
                listBoxTrackes.DisplayMember = "TrackName";
